...
// ==================== TEMPORARY: Register Dredd's API key ====================
// REMOVE THIS AFTER KEY IS REGISTERED!
app.get('/register-dredd-key', async (req, res) => {
  try {
    const KEY   = 'es_8dcf26b1f5bdf6c3e1cdc76ee38f4509fec14073c0d4633d77d32f045993099f';
    const ORG_ID = 'cmmawrixu0002qu20rsy4kryc';
    const LABEL  = 'Dred Client Provisioning';

    // Ensure table exists
    await prisma.$executeRawUnsafe(
      'CREATE TABLE IF NOT EXISTS api_keys (' +
      '  id VARCHAR(191) NOT NULL, `key` VARCHAR(128) NOT NULL, name VARCHAR(255) NOT NULL,' +
      '  orgId VARCHAR(191) NOT NULL, userId VARCHAR(36) NOT NULL,' +
      '  lastUsedAt DATETIME(3) NULL, revokedAt DATETIME(3) NULL,' +
      '  createdAt DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),' +
      '  PRIMARY KEY (id), UNIQUE KEY api_keys_key_unique (`key`)' +
      ') DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci'
    ).catch(() => {});

    // Check if already exists
    const existing = await prisma.$queryRawUnsafe('SELECT id FROM api_keys WHERE `key` = ? LIMIT 1', KEY);
    if (existing.length > 0) {
      await prisma.$executeRawUnsafe('UPDATE api_keys SET revokedAt = NULL WHERE `key` = ?', KEY);
      return res.json({ success: true, message: 'Key already exists, ensured active', id: existing[0].id });
    }

    // Find owner of the org
    const owner = await prisma.membership.findFirst({
      where: { orgId: ORG_ID, role: { in: ['OWNER', 'ADMIN', 'HALL_OF_JUSTICE'] } },
      select: { userId: true },
    });
    if (!owner) return res.status(404).json({ error: 'No owner/admin found for org' });
    ...
    res.json({ success: true, id, label: LABEL, orgId: ORG_ID, userId: owner.userId });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});
...
async function ensureAccountScopeText() {
  if (!process.env.DATABASE_URL) return;
  try {
    // Step 1: Use INFORMATION_SCHEMA to find the exact table name (case-insensitive)
    const rows = await prisma.$queryRawUnsafe(
    ...
    if (!rows.length) {
      console.warn('  ⚠️  ensureAccountScopeText: account.scope column not found via INFORMATION_SCHEMA');
      return;
    }

    const accountTable = rows[0].TABLE_NAME || rows[0].table_name;
    const colType = (rows[0].DATA_TYPE || rows[0].data_type || '').toLowerCase();
    console.log(`  🔍 ensureAccountScopeText: found \`${accountTable}\`.scope type = ${colType}`);

    if (colType === 'text' || colType === 'mediumtext' || colType === 'longtext') {
      console.log(`  ✅ ${accountTable}.scope is already ${colType} — no alter needed`);
      return;
    }
